---
assignee: Optic Dust & David
start: 2026-12-18
due: 2027-01-01
status: In Progress
tags: [city, build, villagers, optic-dust]
---
# Trading Hall Dome (Optic Dust / User)

**Description:**
Finish the massive villager trading hall dome behind the iPhone.

IF multiple villagers get turned into zombies, special measures will need to be taken to cure them 1 at a time after destroying the lecterns and securing them in place to not fuck up the linking.

We have 22 book spots.

---

## 🏗️ Construction Status & Dome Specifications
- **Glass Dome (The "Source of Truth"):** Installed and completely immutable. 
  - **Dimensions:** 98 (W) x 77 (L) x 45 (H).
  - **Generator Specs:** Ellipsoid shape set to 98 Width, 77 Length, 90 Height (height cut in half for the dome). Smoothness slider set to 1.
- **Dome Base:** Installed and finalized.
- **Supporting Connector:** *[NEEDS FIX]* Currently slightly the wrong size. Needs to be adjusted to properly match the elliptical shape of the glass dome above it.
- **Interior Walkway:** The floor walkway inside the glass dome is fully installed.

## 🏛️ Layout, Architecture & Buildings Status
- **Library (Right Side):** Built and nearly complete; villager layout is reaching completion. 2-story building, likely birch wood.
  - *Zombification Status:* Baby zombies installed in floorboards. The design/AI pathing is buggy making curing inconsistent. Holding off on adding more zombies to fix it to prevent excessive noise. We accept the inconsistent zombification for now.
- **Town Hall (Center):** Middle building featuring a large clock tower. Outline is in, roof is installed, and exterior detailing is ~30% complete.
- **Blacksmith (Left Side):** Not started, but location/shape planned. Built with spruce, dark oak, deepslate tiles, or blackstone bricks with metal accents. Houses armor and tools (consider splitting into two separate buildings). 
- **Center Park:** Park area with an elevator entrance coming up from below.
- **Additional Buildings:** 1-story restaurant (left) and 1-story miscellaneous building (right) have not been started yet.

---

## 📊 Trading Hall Library Layout
**Status:** In Dome for library reorganization/restructure. 
**Total Villagers:** 16 Active | 5 Missing/Dead/Future | 21 Total Slots Mapped

### 🛡️ DOWNSTAIRS: Armor & Universals (8 Active, 1 Dead, 1 Future)
*Highly subcategorized by armor piece. Universals isolated on the back wall.*

**🥾 Boots & 🪖 Helmets Section (Front Left Wall - 4 Slots)**
*Left side is Boots, Right side is Helmets*
- ✔️ **Villager 1 (Boots):** Feather Falling IV (**55**/30)
- 💀 **Villager 2 (Boots):** [EMPTY] Reserved for **Depth Strider III** *(Dead - Need to replace)*
- ✔️ **Villager 3 (Helmet):** Respiration III (**23**)
- ✔️ **Villager 4 (Helmet):** Aqua Affinity I (**6**/1)

**👕 Core Armor / Chestplate Section (Front Right Wall - 4 Slots)**
- ✔️ **Villager 5:** Protection IV (**~22**) + Flame (**10**)
- [EMPTY] *(Room for future specialized armor)*
- [EMPTY] *(Room for future specialized armor)*
- [EMPTY] *(Room for future specialized armor)*

**✨ Universal "God Book" Section (Back Middle Wall - 3 Slots)**
- ✔️ **Villager 6:** Mending I (**12**) + Mending I (**12**) *(Double Mending)*
- ✔️ **Villager 7:** Unbreaking III (**30**/5) + Smite II
- 💀 **Villager 8:** [EMPTY] Reserved for **Future Extra Unbreaking III**

---

### ⛏️ UPSTAIRS: Tools & Weapons (8 Active, 3 Dead/Missing)
*Subcategorized perfectly by weapon/tool type.*

**⛏️ Pickaxes & ⚔️ Core Swords (Front Left Wall - 5 Slots)**
- ✔️ **Villager 9 (Pickaxe):** Efficiency V (**23**) + Flame (**19**)
- ✔️ **Villager 10 (Pickaxe):** Fortune III (**26**) + Bane of Arthropods V (**46**) + Impaling IV (**18**)
- ✔️ **Villager 11 (Pickaxe):** Silk Touch I (**8**) + Unbreaking II (**18**)
- ✔️ **Villager 12 (Sword):** Sharpness V (**34**) + Lure III (**36**) *(Casino replacement)*
- ✔️ **Villager 13 (Sword):** Looting III (**15**) *(Casino replacement)*

**🔥 Fire, 🔨 Mace, & 🔱 Spear/Trident Section (Front Right Wall - 4 Slots)**
- 💀 **Villager 14 (Sword/Fire):** [EMPTY] Reserved for **Fire Aspect II** *(Dead - Need to replace)*
- ✔️ **Villager 15 (Mace):** Density V (**21**/1) + Flame (**10**) 
- ✔️ **Villager 16 (Mace/Boots):** Lunge III + Frost Walker I 
- ✔️ **Villager 17 (Trident):** Impaling V

**🏹 Bow Section (Back Middle Wall - 2 Slots)**
- ✔️ **Villager 18:** Power V (**11**) + Channeling I (**45**)
- 💀 **Villager 19:** [EMPTY] Reserved for **Infinity I** *(Missing - Bow)*

**🔱 Trident Overflow Section (Back Right Wall - 2 Slots)**
- ✔️ **Villager 20:** Riptide III (**33**/28) + Thorns (**40**/35) + Mending I (**10**/5)
- 💀 **Villager 21:** [EMPTY] Reserved for **Loyalty III** *(Missing - Trident)*

---

### 🗑️ THE BANISHED / ARCHIVE
*Obsolete villagers removed from the main Library to save space/emeralds. Move to AoT City.*
- **Old Tools:** Bane of Arthropods V (**26**) + Fortune III (**46**) *(Replaced by 29 Fort guy, then swapped again)*
- **Old Sword:** Sharpness V (**44**) *(Replaced by 34 guy)*
- **Old Tools:** Looting III (**27**) + Silk Touch I (**13**) *(Replaced by 15 Looting / 8 Silk Touch guys)*
- **Under Casino Spares:** Mending I (**16**) + Lure III (**17**), Luck of the Sea III (**10**), Channeling I (**10**), Lure III (**15**).

---

### 📖 Lore & Atmosphere
- **Theme:** A segregated, oppressive society where Iron Golems are treated as second-class citizens to Villagers
- **Podiums:** - A grand, high podium on the left for the Villager King (and Wei).
  - A smaller, lower podium on the right for the Iron Golem "politician".

### ⚙️ Technical Notes & Redstone
- This layout stops the bugged 1.21.11 Zombie Horses from spawning AND instantly gives us the 2-block vertical clearance needed for the delivery pipes underneath.
- **Auto-Brewer & Golden Apples:** Need to build an auto-brewer for splash potions of weakness. Also need to hook up a timed dispenser on the sides for golden apples.
- **Apple & Potion Delivery Pipe (Hybrid System):**
  - **Inner Base (Water Streams):** Run water streams on the lower stone slab layer. Place exactly **one block of Packed Ice every 8 blocks** (where the water resets) to maintain item momentum to bring weakness potions to their destination.
  - **Outer Edge (Minecart Handoff):** The last chunk of delivery drops the items onto a Hopper Minecart track on the outer perimeter to finish the process.
  - **Performance & Bug Prevention:** - Map the chunk borders! The minecart track MUST sit entirely within a **single chunk** to prevent Bedrock's asynchronous chunk-unloading bugs from breaking the cart.
    - Hook the minecart to a detector rail so it only runs when an item is actually dispensed (vastly more lag-efficient than using 20-30 hoppers).

![[Pasted image 20260302162814.png]]